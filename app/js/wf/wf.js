		...
		createSecond: function(dataArr) {
			var _this = this,
				result = dataArr.map(function(item, index) {
					...
					return '\
					<div thumbnail="'+item[_this.opts.thumbnail]+'" videoid="'+item[_this.opts.videoid]+'" videourl="'+item[_this.opts.videourl]+'" \
					    like="'+item[_this.opts.likes]+'" class="wf-item wf-item-2" username="'+item[_this.opts.username]+'" \
					    style="padding-bottom: '+height+'; background-image: url('+item[_this.opts.urlField]+');" headphoto="'+item[_this.opts.headphoto]+'">\
						<div class="mark">\
							<img src="'+item[_this.opts.thumbnail]+'">\
							<p style="text-align: right;"><img class="like-icon" src="./images/heart-line.svg">'+item[_this.opts.likes]+'</p>\
						</div>\
					</div>';
				});
			return result.join('');
